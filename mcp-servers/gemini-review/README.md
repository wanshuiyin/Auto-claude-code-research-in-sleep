...
- Antigravity transcript recovery validates conversation ids with `^[0-9A-Za-z_-]+$`, caps ids at 128 characters, requires the per-call invocation nonce, and only considers transcript lines at or after the nonce line. Generated text artifacts are read only from the final planner response, only as a fallback when that response has no substantive inline text, only if their resolved `file://` path stays under `GEMINI_REVIEW_AGY_APP_DATA_DIR/brain/<conversation>/`, and only if the artifact was updated during the current invocation window.
