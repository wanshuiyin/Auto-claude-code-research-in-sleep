#!/usr/bin/env bash
# tools/sync_main_skills.sh — v0.4.25
#
...
RUNTIME_HELPERS=(
    ...
    forensics_gate.py
    # === v0.4.25 additions: helpers newly referenced by synced SKILL.md ===
    # (auto-review-loop stop-condition transition table + its Copilot
    #  host-evidence sibling import; idea-discovery stage gate)
    review_gate.py
    copilot_native_evidence.py
    idea_discovery_gate.py
)
...
# NOTE: this script does NOT auto-prune assets/tools/ — the 32 helpers
# above are the complete intended bundle as of v0.4.25. A stale extra file
# is caught by the v0.4.22 exact-inventory test (asserts assets/tools has
# EXACTLY these 32 entries) on next `cargo test`.
...
echo "     # exact-inventory tests: 32 tools helpers, posterly MIT txt)"
echo
echo "  3. ./target/release/aris --version       # → 0.4.25 (or current after Cargo.toml bump)"
