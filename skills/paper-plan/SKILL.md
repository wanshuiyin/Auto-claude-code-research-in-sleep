...
- **REVIEWER_MODEL = `gpt-5.6-sol`** — Model used via the cross-family reviewer subagent for outline review. Must be an OpenAI model.
...
- Read `../shared-references/press-release-principle.md` when choosing the paper's framing and competitive axis — writer-side claim posture only; never pass it (or mention of it) to reviewer/auditor sub-agents.
...
**Check for `CLAIMS_FROM_RESULTS.md` first:**
- If the file exists and is validated, import its verified claims into the plan.
- If the file is missing or contains `REVIEW_UNAVAILABLE`:
  - In `draft` mode: continue, and tag claims as `[unadjudicated]`.
  - In `submission` mode: stop and ask the user to run `/result-to-claim` first.

Otherwise, extract claims from scratch:
...
Also choose the paper's **competitive axis** now (`../shared-references/press-release-principle.md`):

- Pick the task definition, evaluation dimension, or comparison frame where the evidence actually wins — do not build the narrative on an axis where the paper loses. The axis must be evidence-backed (honesty gates and venue-required comparisons still bind); this is about which winning axis headlines, not about hiding required results.
- **Timing (hard)**: the plan is the LAST point where reframing is allowed. Once `/paper-writing` Phase 1.5 freezes `PAPER_ACCEPTANCE_CONTRACT.md`, downstream phases (paper-write, improvement loops) make sentence-level stance edits only — no new storyline, no competitive-axis change mid-loop.
...
- **Claims-Evidence Matrix is the backbone** — every claim must map to verified evidence; mark unverified claims as "needs experiment".
- **Respect page limits and venue citation norms** — ML conferences: main body to Conclusion end; IEEE venues: references included in page count. Use `natbib` (`\citep`/`\citet`) for ML conferences and numeric `\cite{}` for IEEE.
- **Anonymity preservation** — keep author blocks anonymous during drafting.
- **Concrete figure planning** — specify visual structure, comparative baselines, and data sources for each planned figure.
- **Large file handling** — write large files reliably using standard chunked writes when necessary.
