...
python3 "$RENDER_HTML" idea-stage/IDEA_REPORT.md
# … then the skill skips the reviewer Task subagent step if --no-review
...
**If `should_review` is true**, launch a fresh reviewer Task subagent (NEVER resume; `model: REVIEWER_MODEL` — cross-family max tier per `../shared-references/reviewer-routing.md`) with the prompt below. The reviewer reads the source MD + generated HTML directly; it does **not** see this skill's intermediate state.
...
**Reviewer prompt (mandatory shape).** Send this to a fresh reviewer Task subagent:
...
**If the Task tool is unavailable** (rare): emit `verdict: REVIEW_UNAVAILABLE` to the sidecar, do not fabricate `PASS`, and tell the user the HTML was generated but **not** independently reviewed. The user can manually invoke `/research-review` on the source MD later.
