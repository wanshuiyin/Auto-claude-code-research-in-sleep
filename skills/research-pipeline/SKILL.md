...
> ⚠️ **Chosen-idea handoff**: Pass the selected idea title as the positional argument so `/experiment-bridge` knows which idea's `EXPERIMENT_PLAN.md` to implement. If the user picked a non-top idea at Gate 1, `/idea-discovery` will have re-run `/research-refine-pipeline` for that idea before reaching this stage.

> 💡 **Force queue mode**: For pipeline runs with multi-seed sweeps from the start, append `--- batch: queue` to force `/experiment-queue` for all milestones (passed through to `/experiment-bridge` Phase 4 routing).
