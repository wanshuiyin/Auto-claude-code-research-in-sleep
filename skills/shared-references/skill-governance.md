...
- **A provenance record cannot be same-family.** `stamp()` calls
  ...
  the reviewer are the same model family — e.g. `claude`+`sonnet`, or `gpt-6-astra`+`codex`,
  or the trap `gpt-6-astra`+`oracle-pro` (oracle routes to a GPT-Pro tier, so it is the
