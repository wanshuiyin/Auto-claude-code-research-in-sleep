...
| auto-review-loop subscription budget guard | any `auto-review-loop*` skill starts a loop iteration | `loop_budget.py` (`check`, `record`, `status`) | `~/.aris/usage/loop-usage.jsonl` (one JSONL record per executor/reviewer turn) | Pre-iteration block in `auto-review-loop`, `auto-review-loop-llm`, `auto-review-loop-minimax` | `loop_budget.py record --side {claude\|codex} --skill <skill-name>` after each successful turn | `loop_budget.py check --side {claude\|codex}` (Policy A gate; exit 2 stops the loop, `status` shows headroom) |
