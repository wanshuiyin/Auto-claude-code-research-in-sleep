...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, WebFetch, Agent, Skill, mcp__claude-review__review_start, mcp__claude-review__review_reply_start, mcp__claude-review__review_status
---
...
**What this does:**
- Claude reviewer acts as a grant review panelist (not a paper reviewer)
...
- Timeline: [summary]
- Reviewer feedback: [key points from Claude reviewer]
...
**State**: Write `GRANT_STATE.json` with `phase: 2`, aims summary, and Claude reviewer threadId.
...
**What this does:**
- Claude reviewer acts as a grant review panelist
...
> ⚠️ **claude-review MCP fallback**: If `mcp__claude-review__review_start` is not available (no OpenAI API key), skip external review. Note "External review skipped — no claude-review MCP available. Consider running `/auto-review-loop-llm` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.

If `/research-review` is invoked (preferred), it handles the Claude reviewer call internally. If calling Claude reviewer directly (e.g., to maintain thread context from Phase 2):
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via `mcp__claude-review__review_reply_start`.
...
- Timeline: [N] years
- Review score: [summary from Claude reviewer]
...
| `reviewer model` | claude-review | Claude review model | → claude-review MCP |
