...
description: "Conditionally double-blind bibliographic cross-check (double-blind only when the host can isolate the web agent; a flag-only heuristic when isolation can't be attested): one agent extracts each cited entry verbatim from the local .bib, a second independently re-fetches the same paper's record from the web while blind to the .bib, and a third extractor emits a mechanical field-by-field delta to catch dropped/re-ordered authors, wrong years, wrong venues, and bad DOIs/arXiv ids. The main agent (never a shard) assigns the MATCH/MINOR/MISMATCH verdict per reference, and exports a side-by-side spreadsheet. Use when user says \"citation crosscheck\", \"double-blind citation check\", \"verify bib against scholar\", \"bib field audit\", \"引用交叉核对\", or before submission when bibliography FIELD accuracy (not just existence) must be certified."
...
# Citation Cross-Check (conditionally double-blind bib-vs-web field audit)

> **Codex mirror adaptation (normative).** This skill has no cross-model jury *by
> design* — it is an evidence instrument, not an acquittal step (see the acceptance-gate
> note in Step 2, and pair it with `citation-audit` for the cross-model verdict). A and B
> are read-only extraction shards and C emits a mechanical per-field delta, so every
> verdict is assigned by the main agent from that delta plus its own primary-source
> fetch — not by any shard and not by a reviewer model.
>
> **Blindness is NOT automatically identical across hosts — it is *conditional on
> isolation*, and the gate enforces that.** The whole cross-check rests on B never seeing
> the `.bib`. On the mainline, the `Agent` tool gives B a fresh context. On a Codex host
> the frontmatter grants **no delegation tool** (no `Agent`, no `spawn_agent` — see
> `allowed-tools`), so:
> - **If** the host can dispatch B in a genuinely isolated context that does **not** inherit
>   the main agent's history (which has already read the `.bib` in Step 1) — passing B only
>   the title+first-author list — B may set `blind:true` and the run certifies normally.
> - **Otherwise** (B would run in the main context, or "fresh sequential context" cannot be
>   guaranteed to drop the parent's `.bib` view), B MUST set `blind:false`. The Step-3.5
>   gate then **downgrades every `MATCH`/`MINOR` to `ESCALATE`**: the run still *flags*
>   discrepancies for the main agent's Step-4 webcheck, but it *certifies nothing*. It is a
>   labelled heuristic, not a double-blind certification.
>
> So the mirror never *claims* blindness it cannot deliver: it either has real isolation and
> attests it, or it declares `blind:false` and drops to flag-only. Do not add a
> `spawn_agent`-based reviewer/continuation route — the trust decision is the main agent's
> `WebFetch`, and it must not be delegated to a shard.
...
single reviewer cannot do reliably: an **adversarial, field-level diff** (double-blind when the host can isolate the web agent, a labelled flag-only heuristic otherwise)
...
| Method | one cross-model reviewer | **two extractors (blind when isolation is attested) + a mechanical field-diff** |
...
- **BLIND = conditional** — the web agent (B) MUST be prompted with only each paper's title +
  first author, plus its opaque `dedup_key` (a run-local ordinal that carries no
  bibliographic content), and MUST NOT read the `.bib` or any repo file. B attests
  `blind:true` **only** when the host isolates its context so it cannot inherit the `.bib`
  (see the mirror-adaptation note above); otherwise B attests `blind:false` and the Step-3.5
  gate certifies nothing (flag-only). Do not relax the prompt restriction, and never assert
  `blind:true` without real isolation — an un-isolated `blind:true` is the one thing that
  defeats the whole cross-check.
...
### Step 2 — Run A and B blind to each other
...
and B are two shards that only *extract* records (each returns an `entries[]` set keyed
by `dedup_key` — never by list position, never a B-visible cite key, never a quality
ranking). Each shard returns the `{shard_id, entries:[{dedup_key, …}]}` envelope of
[`../shared-references/fan-out-pattern.md`](../shared-references/fan-out-pattern.md), and the
executor runs the mechanical `dedup_key` join below before C. **No shard assigns a
`MATCH`/`MINOR`/`MISMATCH`/`NEEDS_WEBCHECK` label at all** — the main agent assigns all
four in Step 4 from C's field evidence, and only its own web-confirm edits the `.bib`.

**Dispatch (tier-portable).** On this host there is no Claude `Agent` tool, so the mainline's
Tier 1 (Workflow-concurrent) and Tier 2 (`Agent`) forms both degrade here: run A, then B, then
C one after the other, each in the freshest context the host can give. The join and the
field-diff are identical to the mainline. **Blindness is not automatic here** (see the mirror
adaptation note above): B sets `blind:true` only if its context provably did not inherit the
`.bib`, and `blind:false` otherwise — in which case Step 3.5 downgrades every accept to
`ESCALATE` and the run flags rather than certifies. B is prompted with only the
title+first-author list and is forbidden from reading any local file; but when B is not run in
an isolated context, that prohibition is not *enforced* (B could observe the parent's `.bib`
read, or A's output), which is exactly why an un-isolated B must attest `blind:false` and the
gate then certifies nothing. Isolation is the guarantee; the prompt restriction alone is not.

**Breadth.** The shard set is fixed at three by the *design*, not by a budget: two records are
the minimum for a double-blind diff (one per provenance), and the third is the differ that must
have produced neither. Breadth is not a swept parameter here, so there is no breadth/cost
trade-off to bound — but note the ceiling: two independent records, not an ensemble.
...
> citation set. **Type-A** (executor may self-judge): the `dedup_key` join balanced, every entry
> labelled, the `.xlsx` written. **Type-B** (merit — is the entry bibliographically correct):
> never delegated to a shard. A `MISMATCH` or a `NEEDS_WEBCHECK` resolution requires the main
> agent's own **primary-source** fetch — no fetch, no finding. A `MATCH`/`MINOR` rests on the
> two independent records agreeing, without a third fetch; that is weaker evidence than a
> fetch, so read a no-flag `MATCH` as "two sources concur", not "verified against the
> publisher of record".
>
...
> gate"; this skill takes the second: `scripts/cx_verify.py` (Step 3.5) is an external process
> that decides `MATCH`/`MINOR` by declared normalization and **fails closed** to `ESCALATE`
> otherwise — the same script and the same verdicts on this host as on the mainline.
>
> The limit, stated plainly: the accept path is mechanical and a `MISMATCH` needs the main
> agent's own primary-source fetch, but the executor still reads that page, so this skill does
> **not** deliver cross-model *acquittal* and does not replace one — which is why it is meant to
> run **in addition to** `citation-audit` (a single cross-model reviewer), not instead of it.
> Never report a `MISMATCH`, least of all a fabrication finding, on shard text alone.
...
### Step 3 — Run C, the field-diff extractor (after A+B return)
...
A third pass (a fresh-context run — this host grants no delegation tool) reads both JSONs — it
produced neither — and, on each `dedup_key`-joined pair, diffs the two records field by
field: title (ignore case/braces),
**author set AND order**, year, venue (NeurIPS ≡ "Advances in NIPS"; note a genuinely
different event), volume/pages, arXiv id / DOI. Output `/tmp/cx_C_fielddiff.json`:
...
three tiers." This skill has no cross-model jury, so the gate is an **external process**. It is the
*same script and the same verdicts* on this host as on the mainline — the dispatch of A/B/C
degrades by tier, the gate does not. The gate **recomputes both the field diff and the routing
flags from A's and B's own raw records** and reads B's blindness attestation from B directly;
C's `flags` are advisory only (unioned with the recompute) and a C schema violation fails
closed — so C is not in the trust path for the diff **or** the routing. Resolve `$CX_VERIFY`
with the Codex hybrid chain
(managed project install, in-repo checkout, ARIS repo fallback, then the copied global Codex
skill install). This is **Policy A — skill-local gate**:

> **Resolver note (owner-local, per [`../shared-references/integration-contract.md`](../shared-references/integration-contract.md)).**
> `cx_verify.py` is a single-skill helper under this skill's own `scripts/`, resolved via the
> Codex hybrid chain below. It deliberately ships **no** `tools/cx_verify.py` shim — it has no
> legacy `tools/` entry to forward from (unlike the Phase-3 moves that kept one) — which is the
> owner-local Layer-0 exception the contract's Policy-A row registers.
...
# Global pointer written by smart_update_codex.sh, for hosts with no per-project install.
if [ -z "${ARIS_REPO:-}" ] && [ -f "$HOME/.aris/repo" ]; then
    ARIS_REPO=$(cat "$HOME/.aris/repo" 2>/dev/null) || true
fi
[ -f ".agents/skills/citation-crosscheck/scripts/cx_verify.py" ] && CX_VERIFY=".agents/skills/citation-crosscheck/scripts/cx_verify.py"
[ -z "$CX_VERIFY" ] && [ -f "skills/citation-crosscheck/scripts/cx_verify.py" ] && CX_VERIFY="skills/citation-crosscheck/scripts/cx_verify.py"
[ -z "$CX_VERIFY" ] && [ -n "${ARIS_REPO:-}" ] && [ -f "$ARIS_REPO/skills/citation-crosscheck/scripts/cx_verify.py" ] && CX_VERIFY="$ARIS_REPO/skills/citation-crosscheck/scripts/cx_verify.py"
[ -z "$CX_VERIFY" ] && [ -f ~/.codex/skills/citation-crosscheck/scripts/cx_verify.py ] && CX_VERIFY="$HOME/.codex/skills/citation-crosscheck/scripts/cx_verify.py"
[ -z "$CX_VERIFY" ] && {
  echo "ERROR: cx_verify.py not resolved at .agents/skills/citation-crosscheck/scripts/, skills/citation-crosscheck/scripts/, \$ARIS_REPO/skills/citation-crosscheck/scripts/, or ~/.codex/skills/citation-crosscheck/scripts/." >&2
  echo "       The requirement-(4) gate cannot run; do NOT hand-assign MATCH/MINOR in its place. Fix: rerun install_aris_codex.sh, export ARIS_REPO, or copy the citation-crosscheck skill into ~/.codex/skills/." >&2
  exit 1
}
