...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧾 **`/citation-audit` adds opt-in `--uncited` flag for unused bib entries** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [SKILL.md](skills/citation-audit/SKILL.md)). Pass `--uncited` to surface bib entries that exist in the audited bib file(s) but are not referenced by any `\cite{...}` in the paper body. Detect-only — uncited entries are **not** sent to the cross-model reviewer (no extra web-lookup cost) and **never** alter the top-level `verdict` / `reason_code` / `summary`. **Zero behavior change for existing users**: the optional `details.uncited_entries` JSON field and `## Uncited Entries (opt-in)` MD section are omitted entirely when the flag is unset; downstream consumers (`paper-writing` Phase 6, `tools/verify_paper_audits.sh`) MUST treat field absence as the only valid default state. Suggestion heuristic: `prune` (dead-weight entry; recommend deleting) by default, only emit `check` when there is concrete local evidence (TODO comment in a `.tex` file, recently removed `\cite` in `git diff`) — never infer intent from the bib key string alone. Built via a delegated-agent + maintainer-fixup workflow: the delegated agent ran two rounds of self cross-model review (Codex MCP `gpt-5.5` xhigh) and applied 2 blocker fixes; maintainer's independent codex audit flagged 4 additional required fixes (`skills-codex` mirror sync via `spawn_agent` idiom + `cited_keys`/`bib_keys` protocol-set definitions in Step 2 + `details.total_entries` semantic clarification + reasoning on subtle cited-only Step 3 default-behavior change). Both commits visible in the PR history (delegated-agent commit + maintainer fixup commit) so future contributors can see the workflow in action
