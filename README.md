...
> **Important:** Codex MCP uses the model from `~/.codex/config.toml`, not from skill files. Make sure it says `model = "gpt-5.4"` or another current Codex model such as `gpt-5.3-codex` or `gpt-5.2-codex` — otherwise Codex may fail with "model not found". Run `codex setup` or edit the file directly.
...
| Constant | Default | Description |
|----------|---------|-------------|
| `REVIEWER_MODEL` | `gpt-5.4` | OpenAI model used via Codex MCP. Options: `gpt-5.4`, `gpt-5.3-codex`, `gpt-5.2-codex`, etc. |
...
- [x] **Configurable REVIEWER_MODEL** — all Codex-dependent skills support custom reviewer model (default `gpt-5.4`, also works with `gpt-5.3-codex`, `gpt-5.2-codex`, etc.)
