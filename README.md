...
- **2026-07-03** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧬 **Claude Science borrows: compute-env contract, trigger-rate measurement, output discipline** ([#339](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/339), [#340](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/340), [#341](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/341); adapted from Anthropic's Claude Science skills, Apache-2.0, ported off their proprietary `host.*` runtime). **(1)** [`compute-env-contract.md`](skills/shared-references/compute-env-contract.md): one declarative env spec (ordered `pip_phases` mechanically win version fights) rendered per provider shape, an `env:<name>@<specHash>` content-hash ledger so "did the env change?" is a string comparison, and three-tier validation whose top tier spawns a **fresh subagent that follows the skill's own doc verbatim** and reports every doc-vs-reality divergence — wired into all five compute skills. **(2)** [`tools/meta_opt/trigger_eval.py`](tools/meta_opt/trigger_eval.py): ARIS's first skill-description **trigger-rate measurement** — plan-mode `claude -p` probes score trigger / confusion(→which sibling skill won) / miss under the realistic installed corpus; measure-only, feeds `/meta-optimize` proposals (the confusion matrix is the signal: overlap means disambiguate, not "make it pushier"). **(3)** Output discipline: verification lives in the tool trace and audit artifacts, never as "citations verified" prose; style lints run once, never loop; `paper-figure`'s checklist now separates correctness (always binds — excluded data never enters summaries, captions tested against every plotted row) from overridable style guidance. ⚠️ Run `bash tools/smart_update.sh --apply` to pull.
- **2026-07-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔩 **Reliability wave + LOOPS.md adoption** ([#328](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/328)–[#337](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/337)). **Broken:** on stock macOS (bash 3.2), `smart_update_codex.sh` died on every no-overlay invocation and `smart_update_copilot.sh --apply` exited mid-apply (`set -u` + empty-array expansion); `install_aris.sh --replace-link` had never worked (sed delimiter collided with the plan format); 29 of 31 test files never ran in CI, which is how all of it shipped. **Fixed:** length-guards across six scripts, an awk rewrite for `--replace-link` with regression tests, a full-pytest CI matrix on ubuntu + macOS (the macOS leg is what catches the bash-3.2 class), import-safe MCP servers, and a mirror shared-reference parity guard. **Adopted from Karpathy's LOOPS.md:** reviewers now open adversarial by default ("assume it's broken, prove it"); a *Debugging With Traces* discipline (diff the raw reviewer transcripts at the judgment-divergence point before re-invoking); a restart doctrine (discard-and-reimplement is a peer move to patching, with a hard never-delete-state boundary); [`taste-calibration.md`](skills/shared-references/taste-calibration.md) (weighted axes + human-curated good/bad anchors); `/meta-optimize` gains a harness-diet deletion analysis + a named-bottleneck succession ledger; `/paper-writing` gains **Phase 1.5** — a pre-writing acceptance contract negotiated adversarially with a fresh reviewer and graded at the submission gate. ⚠️ Re-run `bash tools/smart_update.sh --apply` (or your platform's variant) to pull the fixes — macOS users especially.
