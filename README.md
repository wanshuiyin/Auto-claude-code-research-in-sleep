...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🛠️🔗 **Tools-infrastructure migration kicked off**. (a) [`install_aris.sh`](tools/install_aris.sh) creates an optional `.aris/tools -> <aris-repo>/tools` symlink during install / reconcile ([#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192), closes [#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174)) — idempotent, dry-run-aware, safely skips user-modified paths; Phase 0 of the four-step tools-stability plan (#174 → #176 → #177 → #178). **Zero impact** until rerun the installer. 8-case regression test added. (b) [`/experiment-queue`](skills/experiment-queue/SKILL.md) orchestration paths repaired ([#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193)) — first real user of the new symlink. 7 cascading bugs fixed (4 user-reported root causes: wrong `tools/` subdir, no remote `scp` step, `scp + $HOME` SFTP-mode unreliability, `--log` flag silently ignored; plus 3 audit-surfaced edges). Step 3 now splits into helper resolution / remote-path computation / bootstrap+scp / launch-with-`--log-dir` / persisted `run_meta.txt` / resume-existing-queue. Pure prose + docstring; `queue_manager.py` logic untouched. Three rounds of Codex MCP `gpt-5.5` xhigh audit. Windows `install_aris.ps1` parallel update tracked as follow-up
