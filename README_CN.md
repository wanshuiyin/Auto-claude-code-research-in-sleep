...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🛠️🔗 **Tools-infrastructure 迁移启动**。(a) [`install_aris.sh`](tools/install_aris.sh) install / reconcile 时创建可选 `.aris/tools -> <aris-repo>/tools` 符号链接 ([#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192), 关闭 [#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174))——幂等、dry-run-aware、安全跳过用户内容；4 步 tools-stability 迁移（#174 → #176 → #177 → #178）的 Phase 0。**对老用户零影响**直到 rerun installer。8-case 回归测试。(b) [`/experiment-queue`](skills/experiment-queue/SKILL.md) 编排路径修复 ([#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193))——symlink 第一个真实用户。修了 7 个串联 bug（4 用户反馈根因：`tools/` 子目录错、远程 `scp` 步骤缺失、`scp + $HOME` SFTP 模式不可靠、`--log` flag 静默忽略；3 审计发现的边界）。Step 3 现在分 helper 解析 / remote-path 计算 / bootstrap+scp / launch-with-`--log-dir` / 持久化 `run_meta.txt` / resume 现有 queue。纯 prose + docstring；`queue_manager.py` Python 逻辑未动。3 轮 Codex MCP `gpt-5.5` xhigh review。Windows `install_aris.ps1` 平行更新作为 follow-up
