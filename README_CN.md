...
- **2026-07-03** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧬 **Claude Science 借鉴:compute-env 契约、触发率测量、输出纪律**([#339](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/339)、[#340](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/340)、[#341](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/341);改编自 Anthropic 官方 Claude Science skills,Apache-2.0,已脱离其专有 `host.*` runtime)。**(1)** [`compute-env-contract.md`](skills/shared-references/compute-env-contract.md):一份声明式环境 spec(有序 `pip_phases` 机械性赢下版本之争)按 provider 形态渲染,`env:<name>@<specHash>` 内容哈希台账让"环境变没变"变成字符串比对,三层验证的最高层是**派一个全新 subagent 逐字照 skill 自己的文档跑一遍**、报告每一处文档与现实的分歧——已接入全部五个 compute skill。**(2)** [`tools/meta_opt/trigger_eval.py`](tools/meta_opt/trigger_eval.py):ARIS 首个 skill description **触发率量化**——plan 模式 `claude -p` 探针在真实安装列表下打分 触发/认错(→哪个兄弟 skill 抢走了)/漏触发;只测不改,产出喂给 `/meta-optimize` 提案(认错矩阵才是信号:重叠该消歧,不是把话说得更冲)。**(3)** 输出纪律:验证活在工具轨迹和审计产物里,正文永不写"引用已核实";风格 lint 只跑一遍不循环;`paper-figure` 清单分区为正确性(永远约束——被排除数据绝不进汇总、caption 对每条数据成立)与可覆盖的风格默认。⚠️ 运行 `bash tools/smart_update.sh --apply` 拉取。
- **2026-07-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔩 **可靠性修复潮 + LOOPS.md 吸纳**([#328](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/328)–[#337](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/337))。**坏的:**原生 macOS(bash 3.2)上 `smart_update_codex.sh` 每次不带 overlay 调用必崩、`smart_update_copilot.sh --apply` 中途退出(`set -u` + 空数组展开);`install_aris.sh --replace-link` 从上线起就没生效过(sed 分隔符与 plan 格式冲突);31 个测试文件里 29 个从不进 CI——这就是问题存活至今的原因。**修的:**六个脚本加长度守卫、`--replace-link` 用 awk 重写并配回归测试、CI 增加 ubuntu + macOS 双平台全量 pytest(macOS 腿专抓 bash 3.2 这一类)、MCP server import 无副作用、mirror shared-reference parity 守卫。**从 Karpathy LOOPS.md 吸纳:**审稿默认对抗姿态("先假设有问题,去证明它");*Debugging With Traces* 纪律(先 diff 原始审稿 transcript 找判断分歧点,再重新调用);推倒重来 doctrine(推翻重写与打补丁平级,附"状态永不删"硬边界);[`taste-calibration.md`](skills/shared-references/taste-calibration.md)(加权轴 + 人工策划的好/坏锚点);`/meta-optimize` 增加"删脚手架"分析和瓶颈命名台账;`/paper-writing` 新增 **Phase 1.5** —— 写作前与 fresh reviewer 对抗谈判验收合同、投稿门重新按合同打分。⚠️ 请重跑 `bash tools/smart_update.sh --apply`(或对应平台变体)拉取修复——macOS 用户尤其。
