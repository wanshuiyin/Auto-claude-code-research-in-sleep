...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧾 **`/citation-audit` 新增 opt-in `--uncited` flag** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [SKILL.md](skills/citation-audit/SKILL.md))。传 `--uncited` 触发：扫出 bib file(s) 里有但 paper 里没有 `\cite{...}` 引用的死条目。**纯检测、不送 reviewer**——uncited 条目不会触发跨模型 review 调用（无额外 web-lookup 开销），也**绝不**改动 top-level `verdict` / `reason_code` / `summary`。**对老用户零行为变化**：不传 flag 时 `details.uncited_entries` JSON 字段和 `## Uncited Entries (opt-in)` MD 段**完全不存在**；下游 (`paper-writing` Phase 6, `tools/verify_paper_audits.sh`) 必须把字段缺失视为唯一合法默认状态。建议启发式：默认 `prune`（死条目，建议删），只有当存在本地具体证据（某个 `.tex` 里的 TODO 注释、`git diff` 中最近移除的 `\cite`）才标 `check`——绝不从 bib key 字符串本身推断意图。本 PR 通过 delegated-agent + maintainer-fixup 工作流构建：delegated agent 做了两轮 self cross-model review（Codex MCP `gpt-5.5` xhigh）解决 2 个 blocker；maintainer 独立 codex audit 又抓出 4 处 required fix（`skills-codex` mirror 同步用 `spawn_agent` idiom + Step 2 定义 `cited_keys`/`bib_keys` protocol set + `details.total_entries` 语义澄清 + Step 3 cited-only 默认行为变化的论证保留）。两个 commit 都在 PR 历史里可见（delegated-agent commit + maintainer fixup commit），后续 contributor 可以照着学
