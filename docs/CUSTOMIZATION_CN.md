...
| `CODE_REVIEW` | true | GPT-6-Astra 部署前审查实验代码 | → `experiment-bridge` |
...
调部署安全：GPT-6-Astra 代码审查、审查后自动部署、最小实验先跑、并行上限、W&B 日志、base repo URL。
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `CODE_REVIEW` | true | GPT-6-Astra xhigh 部署前审查代码。在浪费 GPU 前抓逻辑 bug |
...
调所有 Codex MCP 调用使用的 reviewer 模型（默认 `gpt-6-astra`），或者 fork SKILL.md 定制 prompt 模板与每个 skill 的工具白名单。
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `REVIEWER_MODEL` | `gpt-6-astra` | Codex MCP 调用的 OpenAI 模型。其他可选：`gpt-5.3-codex`、`gpt-5.2-codex`、`o3`。完整列表见 [supported models](https://developers.openai.com/codex/models/) |
